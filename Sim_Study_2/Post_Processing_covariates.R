...
AIC <- matrix(0, 50, 3)
DIC <- matrix(0, 50, 3)
avg_llik <- matrix(0, 50, 3)

time <- seq(0, 990, 40)
time <- rep(list(time), 150)
...
for(j in 1:4){
  ...
  dirs <- dir(dir)
  nums <- gsub("trace", "", dirs)
  for(i in 1:length(dirs)){
    n_files <- 20
    basis_degree <- 3
    boundary_knots <- c(0, 1000)
    internal_knots <- c(200, 400, 600, 800)
    dir_i <- paste(dir, dirs[i], "/", sep = "")
    Y <- readRDS(paste("./data/data", nums[i], ".RDS", sep = ""))
    X <- matrix(Y$X[,1:i], ncol = i)
    Y <- Y$y
    AIC[i,j] <- FAIC(dir_i, n_files, basis_degree, boundary_knots,
                          internal_knots, time, Y, X = X)
    BIC[i,j] <- FBIC(dir_i, n_files, basis_degree, boundary_knots,
                          internal_knots, time, Y, X = X)
    DIC[i,j] <- FDIC(dir_i, n_files, basis_degree, boundary_knots,
                          internal_knots, time, Y, X = X)
    x <- FLLik(dir_i, n_files, basis_degree, boundary_knots, internal_knots, time, Y, X = X)
    avg_llik[i, j] <- mean(x[4000:20000])
    print(i)
    print(j)
  }
}
...
DIC_choice <- rep(NA, 50)
for(i in 1:50){
  DIC_choice[i] <- which.min(DIC[i,])
}
table(DIC_choice)

AIC_choice <- rep(NA, 50)
for(i in 1:50){
  AIC_choice[i] <- which.min(AIC[i,])
}
table(AIC_choice)

BIC_choice <- rep(NA, 50)
for(i in 1:50){
